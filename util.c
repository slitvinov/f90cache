...
/* log a message to the F90CACHE_LOGFILE location */
void fc_log(const char *format, ...)
{
    va_list ap;
...
/*
  this is like asprintf() but dies if the malloc fails; note that
  we use vsnprintf in a rather poor way to make this more portable
*/
...
void traverse(const char *dir, void (*fn)(const char *, struct stat *))
{
	...
	if (S_ISDIR(st.st_mode)) {
	    traverse(fname, fn);
	    dirlen = strlen(fname);
	    if (strcmp(fname+dirlen-4,".dir") == 0) {
		rmdir(fname);
		free(fname);
		continue;
	    }
